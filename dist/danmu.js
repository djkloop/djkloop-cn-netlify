...
window.onload = function () {
    ...
    var all_row_array = [];
    // 辅助map
    var all_row_finish_helper = {};
    var all_row_finish_runner_helper = {};
    var init_all_row_array = [];
    ...
    var array_map = {};
    var runner_idx_array = []; // 赛跑者对应的分组
    var g_runner_idx_manage = 0; // 所有的赛跑者idx分配中心
    var g_wish_number = 0;
    var init_runner_for_data = function (runners) {
    ...
    //
    var control_per_number = function () {
        //控制屏幕占有率
        document
    ...
    var run_finish = function (ev) {
        ...
        var runner_idx = _$target.getAttribute('data-index'), row_idx = _$target.getAttribute('data-row');
    ...
    var init_runner = function (idx, runner, $document) {
        if (!$document) {
            ...
            if (runner.type === 2) {
                var _$div_content_box = document.createElement('div');
                var _$div_img_box = document.createElement('div');
                var _$div_content = document.createElement('div');
                var _$div_img = document.createElement('div');
    ...
    var get_runner = function () {
        ...
        if (row_runner_length > 0) {
            ...
            var runner = init_runner(runner_idx_array[runner_idx], runner_data, document.querySelector('.danmu-container-lists >.danmu-list-item[data-finish="true"][data-type="' + runner_data.type + '"]'));
    ...
    var go_run = function (idx, ele, runner) {
        ...
        var runner_type = Number(ele.dataset.type);
        var runner_text_len = Number(ele.dataset.text);
        console.log(runner_type, ele.style.cssText);
        if (runner_type === 2) {
            var _color_setting = color_setting('wish');
            var $_child_img = ele.querySelector('.danmu_list_item_img');
            var $_child_content = ele.querySelector('.danmu_list_item_content');
        ...
        var _className = 'unit danmu-list-item ';
        if (play_count == 0) {
        ...
        var speed = ((ele_width + container_width) / duration);
        var show_time = ((ele_width) / speed);
        var next_delay;
        if (per_row_number < 20) {
    ...
    var match_row_runner = function (row_idx, runners) {
        // 跑到和赛道进行匹配
        var row_runner_idx;
        var RunnterItems = all_row_array.filter(function (runnterItem, i) {
    ...
    var row_runner = function (row_idx, runner) {
        if (runner === void 0) { runner = {}; }
        if (row_idx === -1) {
...
var debounce = function (fn, delay) {
    var args = arguments, context = this, timer = null;
    return function () {
        if (timer) {
            clearTimeout(timer);
